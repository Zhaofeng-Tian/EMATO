...
class BNLP:
    def __init__(self, param, traj_l, xc):
        ...
        self.p = ca.vertcat(self.XS, self.XE, self.SL, self.VL, self.gdec)
        ...
        self.res = np.concatenate((self.traj_sl[:self.N]-self.dinit ,\
                                    self.traj_vl[:self.N], \
                                    np.zeros(self.N), \
                                    1.1*np.ones(self.N), \
                                    0.2*np.ones(self.N), \
    ...
    def update(self, xc, traj_l, if_dynamic_vr = True):
        self.xc = xc
        self.traj_sl, self.traj_vl, self.traj_al = traj_l
        print(" Leading car s: ", self.traj_sl[0])
        ...
        print("******************* traj_sl: ", self.traj_sl[-1])
        print("******************* se: {}, ve: {}, ae: {} ".format(se,ve,ae))
        ...
        if self.use_gd_prediction:
            ...
            self.p_value = ca.vertcat(self.xs, self.xe, self.traj_sl[:self.N], self.traj_vl[:self.N], p_gd)
        else:
            # self.p_value = ca.vertcat(self.traj_sl[:self.N], self.traj_vl[:self.N], xc[0], xc[1], xc[-1])
            self.p_value = ca.vertcat(self.xs, self.xe, self.traj_sl[:self.N], self.traj_vl[:self.N], xc[-1])
...
class BNLP_R:
    def __init__(self, param, traj_l, xc):
        ...
        Concat all the defined constraints
        """
        self.g = ca.vertcat(self.gacc, self.gfa,*self.gfv,*self.gfd, *self.gfj, self.gvs, self.gas, self.gss,self.gse)
        self.lbg = np.concatenate((np.zeros(5*self.N),-30*np.ones(1)))
        self.ubg = np.concatenate((self.dmax * np.ones(self.N),np.zeros(4*self.N), 30*np.ones(1) ))
